...
export default defineConfig((ctx) => {
  return {
    ...
    devServer: {
      ...
      port: process.env.QCLI_DEV_PORT ? parseInt(process.env.QCLI_DEV_PORT, 10) : 9000,
      proxy: {
        '/api': {
          target: process.env.QCLI_API_URL || 'http://localhost:3000',
          changeOrigin: true,
        },
        '/db': {
          target: process.env.COUCHDB_URL
            ? process.env.COUCHDB_URL.replace(/\/humanitysync$/, '')
            : 'http://localhost:5984',
          changeOrigin: true,
